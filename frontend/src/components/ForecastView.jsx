...
const ForecastView = ({ forecast, loading, locationName }) => {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in">
      ...
      {/* Location header — shown once at the top */}
      {locationName && (
        <div className="flex items-center gap-2">
          <MapPin size={16} className="text-slate-400" />
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Selected / Current Location</p>
            <p className="text-lg font-black text-slate-800">{locationName}</p>
          </div>
        </div>
      )}
